...
def main():
    ...
    W = solve_for_affine_weights(pred2d_v, gt2d_v, stride=5)
    ...
    W = solve_for_affine_weights(pred2d_v[good], gt2d_v[good], stride=5)

    np.save(f'{NTU_ROOT}/huge8_to_kinect.npy', W)
    return W
...
def load_matching_gt2d_and_latent_pred2d():
    ...
    pred_paths = spu.sorted_recursive_glob(f'{NTU_ROOT}/pred_36161de1_aug/**/*.pkl')
    pred_video_ids = [ntu_main.get_video_id(p) for p in pred_paths]
    ...
    preds2d = np.array([
        camera.camera_to_image(pred) / np.nanmax(box[2:4]) * proc_side
        for (preds, boxes, gts), camera in
        zip(spu.progressbar(poses3d_boxes_gt_per_video), camera_per_video)
    ...
    w1 = np.load(f'{DATA_ROOT}/skeleton_conversion/huge8_48_effv2l_6e-1_chirnozero.npz')['w1']
    pred2d_latent = geom3d.convert_pose(pred2d_v, w1)
