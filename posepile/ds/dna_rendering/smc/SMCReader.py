...
class SMCReader:
        ...
        if isinstance(Frame_id, (str, int)):
            ...
            if Frame_id is None:
                Frame_id_list = sorted(
                    [int(l) for l in self.smc[Camera_group][Camera_id][Image_type].keys()])
            elif isinstance(Frame_id, list):
                Frame_id_list = Frame_id
            rs = []
    ...
    def get_SMPLx(self, Frame_id=None):
        ...
        if Frame_id is None:
            ...
            Frame_id = int(Frame_id)
            assert Frame_id < t_frame, f'Invalid frame_index {Frame_id}'
            frame_list = Frame_id
        else:
            raise TypeError('frame_id should be int, list or None.')

        def get_data(key):
            array = self.smc['SMPLx'][key][()]
            if array.shape[0] == 1:
                return array[0]
            try:
                return array[frame_list]
            except IndexError:
                assert key == 'expression' or np.all(array == 0)
                return np.zeros_like(array[0])

        smpl_dict = {k: get_data(k) for k in ['betas', 'expression', 'fullpose', 'transl']}
