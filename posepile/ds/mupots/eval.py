...
def main():
    ...
    parser.add_argument('--root-last', action=spu.argparse.BoolAction)
    ...
    metrics_wacv = eval_wacv23(FLAGS.pred_path, all_true2d, all_true3d, all_true3d_univ)
...
def get_all_gt_poses():
    ...
    for i_seq in range(n_seq):
        anno_path = f'{DATA_ROOT}/mupots/TS{i_seq + 1}/annot.mat'
        annotations = util.load_mat(anno_path).annotations
...
def get_all_pred_poses(path):
    ...
    n_seq = 20
    n_frames_per_seq = [
        len(glob.glob(f'{DATA_ROOT}/mupots/TS{i_seq + 1}/img_*.jpg'))
        for i_seq in range(n_seq)]
...
def eval_wacv23(pred_path, all_true2d, all_true3d, all_true3d_univ):
    all_pred2d, all_pred3d = get_all_pred_poses(pred_path)
    (matched_pred3d, matched_true3d, matched_true3d_univ, n_annotated_people, n_matched_people
     ) = match_all_poses(all_pred2d, all_pred3d, all_true2d, all_true3d, all_true3d_univ)

    all_pred3d = matched_pred3d
    all_true3d = matched_true3d
    ...
    recall = 1  # np.sum(n_matched_people) / np.sum(n_annotated_people)
    ...
    return np.array(
        [np.mean(dist),
         ...
         np.mean(dist_abs),
         # PCK all
         recall * get_pck(dist / 50),
         recall * get_pck(dist / 100),
         recall * get_pck(dist / 150),
         recall * get_auc_real(dist / 50),
         recall * get_auc_real(dist / 100),
         recall * get_auc_real(dist / 150),
         # CPS all
         recall * get_pck(worst_dist / 100),
         recall * get_pck(worst_dist / 150),
         recall * get_pck(worst_dist / 200),
         recall * get_auc_real(worst_dist / 100),
         recall * get_auc_real(worst_dist / 150),
         recall * get_auc_real(worst_dist / 200),
         # APCK all
         recall * get_pck(dist_abs / 150),
         recall * get_pck(dist_abs / 300),
         recall * get_auc_real(dist_abs / 150),
         recall * get_auc_real(dist_abs / 300),
         ])
